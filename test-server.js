...
if (watch) {
    ...
    server.on('request', function (req, res) {
        ...
        if (req.url === '/two') {
            // res.statusCode = 301;
            // res.setHeader('Location', '/three');
            return res.end(`<script>location.href = '/three'</script>`);
        }

        if (req.url === '/three') {
            res.statusCode = 301;
            res.setHeader('Location', '/four');
            return res.end('three -> four');
        }
...
<body>
    <div>test <span>test span</span></div>
    <div class="agent"></div>
